...
def populateFingerTable(index,incomingId,data,distance):
	global chord_table
	for x in range(index,-1,-1):
		if chord_table[x]['id'] !=-1 and distance > chord_table[x]['distance'] :
			break
		else:
			chord_table[x]['id'] = int(incomingId)
			chord_table[x]['tcp_port']=int(data['tcp_port'])
			chord_table[x]['tcp_ip']=data['tcp_ip']
			chord_table[x]['distance']=int(distance)
	# if data['type'] !='PING':
	# 	multicastSend('PING')
...
def chord_implementor(data):
	...
	if chord_table[probableIndex]['id'] ==-1 :
		populateFingerTable(probableIndex,incomingId, data,distance)
	else:
		oldDistance = chord_table[probableIndex]['distance']
		if oldDistance > distance:
			populateFingerTable(probableIndex,incomingId,data,distance)
	if data['type']!='PING':
		multicastSendThread= threading.Thread(name='multicastSend', target=multicastSend,args=('PING',))
